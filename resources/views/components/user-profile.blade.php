@if (empty($detail_acara))
    <a href="{{ route('profile.show', $detail_acara->owner) }}">
    @elseif ($detail_acara)
    <a href="{{ route('profile.show', $detail_acara->owner) }}">
        @else
        <a href="profile/1">
...
<div class="flex items-center my-5 gap-5">
    ...
    <div>
        <h1 class="flex items-center gap-2">
            @if (empty($detail_acara))
                {{$detail_acara->getRelations()["owner"]->name }}
            @else
                {{$detail_acara->getRelations()["owner"]->name}}
            @endif
            ...
            @if (empty($detail_acara))
                @if (Str::contains($detail_artikel->judul, ['Kabupaten', 'Provinsi']))
                    <span>
                ...
                @endif
            @else
                @if (Str::contains($detail_acara->judul, ['Kabupaten', 'Provinsi']))
                    <span>
                        <svg class="text-blue-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                            viewBox="0 0 24 24">
                            <path fill="currentColor"
        ...
        @if (empty($detail_acara))
            ...
            @if (Str::contains($detail_acara->getRelations()["owner"]->name, ['Kota', 'Kabupaten']))
